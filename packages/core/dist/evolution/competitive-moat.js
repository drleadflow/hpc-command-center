...
export function generateUsageReport() {
    try {
        // Total conversations
        const convRow = db().prepare('SELECT COUNT(*) as count FROM conversations').get();
        const totalConversations = convRow.count;
        // Total tool calls
        const toolRow = db().prepare('SELECT COUNT(*) as count FROM tool_calls').get();
        const totalToolCalls = toolRow.count;
        // Total memories
        const memRow = db().prepare('SELECT COUNT(*) as count FROM memories').get();
        const totalMemories = memRow.count;
        // Total learned skills
        const skillRow = db().prepare("SELECT COUNT(*) as count FROM skills WHERE source = 'learned'").get();
        const totalSkillsLearned = skillRow.count;
        // Total evolution events
        let totalEvolutionEvents = 0;
        try {
            const evoRow = db().prepare('SELECT COUNT(*) as count FROM evolution_events').get();
        ...
        // Top skills by usage
        const topSkills = db().prepare(`SELECT name, total_uses as uses FROM skills
       WHERE total_uses > 0
       ORDER BY total_uses DESC LIMIT 5`).all();
        // Top tools by usage
        const topTools = db().prepare(`SELECT tool_name as name, COUNT(*) as uses FROM tool_calls
       GROUP BY tool_name
       ORDER BY uses DESC LIMIT 5`).all();
        // Longest streak
        let streakDays = 0;
        try {
            const streakRow = db().prepare('SELECT MAX(longest_streak) as maxStreak FROM streaks').get();
        ...
        let totalXp = 0;
        try {
            const xpRow = db().prepare("SELECT total_xp as totalXp FROM user_profile WHERE id = 'default'").get();
...
function generateInsight(conversations, toolCalls, memories, skillsLearned) {
...
//# sourceMappingURL=competitive-moat.js.map