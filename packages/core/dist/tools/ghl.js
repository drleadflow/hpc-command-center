...
function ghlHeaders() {
    ...
    if (!key)
        throw new Error('GHL_API_KEY not configured. Add it to your .env file.');
    return {
...
async function ghlFetch(path, locationId) {
...
registerTool({
    ...
    try {
        ...
        if (status !== 'all')
            path += `&status=${status}`;
        const data = await ghlFetch(path, locationId);
...
registerTool({
    ...
    try {
        ...
        const data = await ghlFetch(`/contacts/search?query=${encodeURIComponent(query)}&limit=${limit}`, locationId);
...
registerTool({
    ...
    try {
        const locationId = input.locationId;
        const data = await ghlFetch('/opportunities/pipelines', locationId);
        const pipelines = data.pipelines ?? [];
        const display = pipelines.length > 0
            ? pipelines.map(p => `📊 ${p.name}: ${(p.stages ?? []).map(s => s.name).join(' → ')}`).join('\n')
            : 'No pipelines found.';
...
registerTool({
    ...
    try {
        ...
        const data = await ghlFetch(`/conversations/search?limit=${limit}&sortBy=last_message_date&sortOrder=desc`, locationId);
        const convos = data.conversations ?? [];
        let unanswered = 0;
