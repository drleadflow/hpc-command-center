...
function getReadySteps(workflow, completedSteps) {
...
function buildStepContext(step, previousResults) {
...
export async function runWorkflow(workflowId, input) {
    ...
    while (true) {
        ...
        const results = await Promise.allSettled(readySteps.map(async (step) => {
            ...
            if (step.condition) {
                ...
                if (!conditionMet) {
                    return { stepId: step.id, status: 'skipped', output: `Condition not met: ${step.condition}`, cost: 0, durationMs: 0 };
                }
            }
        ...
        if (completedSteps.size + failedSteps.size >= workflow.steps.length)
            break;
    }
...
function evaluateCondition(condition, results) {
    ...
    if (stepMatch && results[stepMatch[1]]?.status !== 'completed')
        return false;
    ...
    if (scoreMatch) {
        ...
        if (foundScore && parseInt(foundScore[1], 10) <= threshold)
            return false;
    }
    return true;
}
