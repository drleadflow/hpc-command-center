...
export function getAchievements() {
    const unlocked = db().prepare(`SELECT id, unlocked_at as unlockedAt FROM achievements`).all();
...
export function unlockAchievement(achievementId) {
    ...
    if (!def)
        return null;
    const existing = db().prepare(`SELECT id FROM achievements WHERE id = ?`).get(achievementId);
    if (existing) {
        // Already unlocked
        return { ...def, unlockedAt: undefined };
    }
    const ts = now();
    db().prepare(`INSERT INTO achievements (id, name, unlocked_at) VALUES (?, ?, ?)`).run(achievementId, def.name, ts);
    return { ...def, unlockedAt: ts };
}
export function checkAchievements() {
    const newlyUnlocked = [];
    const alreadyUnlocked = new Set(db().prepare(`SELECT id FROM achievements`).all().map(a => a.id));
    ...
    if (!alreadyUnlocked.has('first-job')) {
        const row = db().prepare(`SELECT COUNT(*) as count FROM jobs WHERE status = 'completed'`).get();
        if (row.count > 0) {
    ...
    if (!alreadyUnlocked.has('streak-7')) {
        const row = db().prepare(`SELECT COUNT(*) as count FROM streaks WHERE longest_streak >= 7`).get();
        if (row.count > 0) {
    ...
    if (!alreadyUnlocked.has('streak-30')) {
        const row = db().prepare(`SELECT COUNT(*) as count FROM streaks WHERE longest_streak >= 30`).get();
        if (row.count > 0) {
    ...
    if (!alreadyUnlocked.has('team-3')) {
        const row = db().prepare(`SELECT COUNT(*) as count FROM employees WHERE active = 1`).get();
        if (row.count >= 3) {
    ...
    if (!alreadyUnlocked.has('team-all')) {
        ...
        const active = db().prepare(`SELECT COUNT(*) as count FROM employees WHERE active = 1`).get();
    ...
    if (!alreadyUnlocked.has('tools-10')) {
        const row = db().prepare(`SELECT COUNT(DISTINCT tool_name) as count FROM tool_calls`).get();
        if (row.count >= 10) {
    ...
    if (!alreadyUnlocked.has('level-5')) {
        const row = db().prepare(`SELECT level FROM user_profile WHERE id = 'default'`).get();
        if (row && row.level >= 5) {
