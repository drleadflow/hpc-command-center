...
export async function runEvolutionCycle() {
    ...
    for (const event of events) {
        try {
            db().prepare(`INSERT INTO evolution_events (id, type, description, before_value, after_value, impact, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).run(crypto.randomUUID(), event.type, event.description, event.before ?? null, event.after ?? null, event.impact ?? null, event.timestamp);
        }
        catch (err) {
...
async function analyzeRecentFailures() {
    ...
    let errorMemories;
    try {
        errorMemories = memoryStore.search('error_pattern', 50);
    }
    catch {
        errorMemories = memoryStore.getAll(100)
    ...
    for (const [pattern, count] of Object.entries(errorGroups)) {
        if (count >= 3) {
            ...
            try {
                ...
                if (!config.apiKey)
                    continue;
                const response = await callModel(config, 'You are a skill improvement advisor. Given a recurring error pattern, suggest a concise system prompt addition (2-3 sentences) that would help an AI agent handle this error better. Respond with ONLY the suggested text, no explanation.', [{ role: 'user', content: `This error has occurred ${count} times:\n\n${pattern}` }], [], 512);
...
function groupSimilarErrors(contents) {
    const groups = {};
...
// ============================================================
async function optimizePopularSkills() {
    const events = [];
    const popularSkills = db().prepare(`SELECT name, description, system_prompt, success_rate, total_uses
     FROM skills WHERE total_uses >= 10 AND success_rate < 0.7
     ORDER BY total_uses DESC LIMIT 5`).all();
    for (const skill of popularSkills) {
        try {
            const config = resolveModelConfig('claude-haiku-4-20250514');
            if (!config.apiKey)
                continue;
            const response = await callModel(config, `You are a prompt optimization expert. Improve this system prompt to increase its success rate. The current success rate is ${(skill.success_rate * 100).toFixed(0)}% across ${skill.total_uses} uses. Return ONLY the improved system prompt, nothing else.`, [{ role: 'user', content: `Skill: ${skill.name}\nDescription: ${skill.description}\n\nCurrent system prompt:\n${skill.system_prompt}` }], [], 1024);
...
// (d) LEARN FROM SUCCESSFUL PATTERNS
// ============================================================
function learnSuccessfulPatterns() {
    const events = [];
    // Query recent successful tool call chains from the tool_calls table
    const recentConversations = db().prepare(`SELECT conversation_id, tool_name, success
     FROM tool_calls
     WHERE success = 1
    ...
    for (const [sequence, count] of sequenceCounts) {
        if (count >= 3) {
            ...
            if (!alreadyKnown) {
                memoryStore.save(`Successful tool pattern (used ${count} times): ${sequence}`, 'skill_result', ['pattern', ...sequence.split(' -> ').map(s => s.trim())], 'evolution');
                events.push({
