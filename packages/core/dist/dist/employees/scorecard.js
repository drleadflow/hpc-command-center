...
export function getScorecardStatus(value, metric) {
...
export function recordMetric(employeeId, metricId, value) {
    const id = crypto.randomUUID();
    // Status will be computed on read when we have the metric definition
    db().prepare(`INSERT INTO scorecard_entries (id, employee_id, metric_id, value, status, recorded_at)
     VALUES (?, ?, ?, ?, ?, ?)`).run(id, employeeId, metricId, value, 'green', now());
}
export function getScorecard(employeeId) {
    const rows = db().prepare(`SELECT id, employee_id as employeeId, metric_id as metricId, value, status, recorded_at as recordedAt
     FROM scorecard_entries
     WHERE employee_id = ?
     ORDER BY recorded_at DESC`).all(employeeId);
    return rows;
}
export function formatScorecard(entries, employee) {
    if (entries.length === 0) {
