...
import { cloneRepo, createBranch, commitAndPush, commitIncremental, createPullRequest, commentOnPR, parseRepoUrl, isDockerAvailable, createContainer, startContainer, execInContainer, stopContainer, removeContainer, } from '@blade/docker-runner';
import { jobs, jobLogs } from '@blade/db';
import { logger } from '@blade/shared';
import { runAgentLoop } from '../agent-loop.js';
import { registerTool, createToolScope, registerScopedTool, getScopedToolDefinitions, destroyToolScope } from '../tool-registry.js';
// ============================================================
// Helpers
// ============================================================
...
function updateStatus(jobId, status, message, extra, onStatus) {
...
function detectTestCommand(repoDir) {
    try {
...
function createLocalAdapter(workDir) {
    return {
        async exec(command) {
...
function createDockerAdapter(container) {
    return {
        async exec(command) {
...
function registerCodingTools(adapter, scopeId) {
    const register = scopeId
        ? (def, handler) => registerScopedTool(scopeId, def, handler)
        : registerTool;
    const readFileDef = {
        name: 'read_file',
...
// ============================================================
function buildSystemPrompt(title, description) {
...
// ============================================================
function buildPRBody(repoDir, title, result, testsPassed) {
...
function buildAgentLogComment(jobId, result, fixResults, testOutput, testsPassed) {
    const lines = [
        '## Agent Activity Log',
        '',
        `**Job ID:** \`${jobId}\``,
...
export async function runCodingPipeline(params) {
    const { jobId, title, description, repoUrl, baseBranch, agentModel, githubToken, onStatus, } = params;
    ...
    try {
        ...
        try {
            const logComment = buildAgentLogComment(jobId, agentResult, fixResults, lastTestOutput, testsPassed);
            await commentOnPR({
                owner,
                repo,
...
//# sourceMappingURL=coding-pipeline.js.map