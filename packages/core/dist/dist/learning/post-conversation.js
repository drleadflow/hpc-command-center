 ...
 */
export async function extractLearnings(conversationId, messages, toolCalls) {
    try {
        ...
        const response = await callModel(config, EXTRACTION_SYSTEM_PROMPT, [{ role: 'user', content: transcript }], [], 2048);
...
function buildTranscript(messages, toolCalls) {
    const parts = ['## Conversation Transcript\n'];
...
function parseLearnings(text) {
    try {
        ...
        return parsed.filter((item) => typeof item === 'object' &&
            ...
            ['preference', 'fact', 'error_pattern'].includes(item.type)).map(item => ({
            type: item.type,
...
function saveOrReinforce(learning, source) {
    ...
    if (duplicate) {
        ...
        memoryStore.save(learning.content, learning.type, learning.tags, `conversation:${source}`);
        logger.debug('Learning', `Saved new ${learning.type} memory`);
    }
}
//# sourceMappingURL=post-conversation.js.map