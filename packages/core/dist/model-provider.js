...
function toOpenAIMessages(systemPrompt, msgs) {
    const result = [
...
function mapOpenAIStopReason(finishReason) {
    switch (finishReason) {
...
async function callOpenAI(config, systemPrompt, messages, tools, maxTokens) {
...
export async function callModel(config, systemPrompt, messages, tools, maxTokens) {
...
// ============================================================
export async function* streamModel(config, systemPrompt, messages, tools, maxTokens) {
    if (config.provider === 'openai') {
...
async function* streamOpenAI(config, systemPrompt, messages, tools, maxTokens) {
    const client = createOpenAIClient(config);
    const openaiMessages = toOpenAIMessages(systemPrompt, messages);
    const params = {
        model: config.modelId,
...
// ============================================================
async function callClaudeCli(systemPrompt, messages, tools, _maxTokens) {
    ...
    try {
        ...
        try {
            unlinkSync(join(tmpDir, 'prompt.txt'));
        }
        catch { /* ignore */ }
 ...
 */
export function resolveSmartModelConfig(complexity = 'standard') {
