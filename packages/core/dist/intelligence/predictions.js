...
function getMemoriesForEmployee(employeeId) {
    try {
        return db().prepare(`SELECT content, created_at as createdAt, tags_json as tagsJson
       FROM memories
       WHERE source = ? OR tags_json LIKE ?
       ORDER BY created_at DESC
       LIMIT 200`).all(employeeId, `%"${employeeId}"%`);
    }
    catch {
        return [];
    }
...
function recentEntries(entries, metricId, n) {
    return entries
...
function projectDaysToTarget(values, target, direction) {
    if (values.length < 2)
        return null;
    ...
    if (direction === 'higher' && rate <= 0)
        return null;
    if (direction === 'lower' && rate >= 0)
        return null;
...
// ---------------------------------------------------------------------------
function closerPredictions(employeeId, entries, def) {
    const preds = [];
...
function operatorPredictions(employeeId, entries, def) {
    const preds = [];
    // Revenue trajectory
...
function wellnessCoachPredictions(employeeId, entries, def) {
    const preds = [];
    // Sleep trend
    ...
    if (hoursToday() >= 21) {
        const memories = getMemoriesForEmployee(employeeId);
        const recentWorkMentions = memories.filter(m => daysAgo(m.createdAt) < 1 && /work|task|deadline|meeting|code|build/i.test(m.content));
...
function wealthStrategistPredictions(employeeId, entries, def) {
    const preds = [];
    // Savings projection
...
function connectorPredictions(employeeId, _entries, _def) {
    const preds = [];
    const memories = getMemoriesForEmployee(employeeId);
...
function reflectorPredictions(employeeId, entries, def) {
    const preds = [];
...
function genericPredictions(employeeId, entries, def) {
    const preds = [];
...
// Public API
// ---------------------------------------------------------------------------
export function generatePredictions(employeeId) {
...
export function dismissPrediction(id) {
    dismissed.add(id);
}
export function formatPredictions(predictions, limit = 3) {
